...
- **Design Phase** — 기획 문서(PDF, 이미지, URL, 텍스트)를 분석하여 개발 태스크 자동 생성
- **Dev Phase** — 태스크별 코드 작성 → 테스트 → 코드 리뷰 사이클 자동 실행 (최대 3회 재시도)
- **Git Management** — Git Worktree 기반 격리 개발 환경, Commit/Push/Pull UI 제공
- **Legacy Analysis** — 기존 프로젝트를 분석하여 프레임워크별 가이드맵 생성
- **Runtime Error Tracking** — 실행 중 발생한 에러 추적 및 자동 수정
- **실시간 업데이트** — WebSocket 기반 에이전트 실행 로그 스트리밍
